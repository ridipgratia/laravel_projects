        ...
        await $.ajax({
            ...
            success: function (result) {
                ...
                for (var i = 0; i < result.message.length; i++) {
                    dataTable.row.add([(i + 1), result.message[i].request_id, result.message[i].date_of_submit, `<button id='add_FTO' class="btn btn-primary" value="${result.message[i].id}">` + result.message[i].action_btn + `</button>`]).draw(false);
                }
            },
            error: function (data) {
                console.log(data);
